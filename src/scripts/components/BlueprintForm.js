...
class BlueprintForm extends Component {
  ...
  attachListeners() {
    ...
    $(this.el).on(
      "change",
      ".quickstart-only.unique-values.abilities select",
      function (e) {
    ...
    // Swap archetype rankings when select value changes
    $(this.el).on(
      "change",
      ".quickstart-only.unique-values.archetypes select",
      function (e) {
        let oldValue = $(this).attr("archetype");
        let newValue = $(this).val();
        let elements = $(this)
          .closest(".quickstart-only")
          .find("select[archetype='" + newValue + "']");
        elements.attr("archetype", oldValue);
        elements.val(oldValue);
        $(this).attr("archetype", newValue);
      }
    );
